...
import re
num=r'(-?\d\.\d+e[-+]\d{2})'
pat=re.compile(r'^(p_temp|p_vdd|p_vc|p_vcrs|f_mhz|swing) = '+num+r'\s*$', re.I)
BAUD=600.6
for c in ('tt','ss','ff'):
    ...
    for vc in (0.60,0.85):
      for t in (-40,27,125):
        for v in (1.08,1.20,1.32):
            g=sorted([r for r in rows if r['p_temp']==t and abs(r['p_vdd']-v)<1e-6 and abs(r.get('p_vc',0)-vc)<1e-6], key=lambda r:r['p_vcrs'])
            if not g: continue
            seed=None
            ...
            print(f"  vctrl {vc:.2f}  {t:>4}C {v:.2f}V  range {rng:>16}   vcoarse@baud "+(f"{seed:.3f} V" if seed else "NOT REACHED"))
PY
