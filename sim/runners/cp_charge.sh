#!/bin/bash
#   setsid nohup sim/runners/cp_charge.sh > sim/results/cp_charge.out 2>&1 &
set -u
cd /home/ttuser/ssh_analog/ct-worktree/sim
./run_corners.sh cp_charge.spice cp_charge 'p_wn|p_ivp' >/dev/null 2>&1
cd results
python3 - <<'PY'
