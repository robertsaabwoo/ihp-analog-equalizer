#!/bin/bash
#   setsid nohup sim/runners/cp_dyn2_run.sh > sim/results/cp_dyn2.out 2>&1 &
set -u
cd /home/ttuser/ssh_analog/ct-worktree/sim
./run_corners.sh cp_dyn2.spice cp_dyn2 'p_temp|p_vdd' >/dev/null 2>&1
cd results
python3 - <<'PY'
import re
num=r'(-?\d\.\d+e[-+]\d{2})'
for c in ('tt','ss','ff'):
    ...
    cur={}
    for line in open(f'cp_dyn2_{c}.log'):
